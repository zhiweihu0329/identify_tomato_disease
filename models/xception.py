...
MODEL_WEIGHTS_PATH = "/home/science/huzhiwei/disease/model/16_xception.hdf5"
...
def train():
    ...
    with tf.device('/cpu:0'):
        model_checkpoint = ModelCheckpoint(str(BATCH) + '_xception.hdf5', monitor='val_loss', verbose=1, save_best_only=True,
                                           mode='min')
...
def evaluate_single_class():
    ...
    for i in range(3):
        print(i+7)
        test_data, test_label = get_one_label_test_data(i+7)
        model = xception()
        model.load_weights(MODEL_WEIGHTS_PATH)
        model.compile(optimizer=Adam(lr=LEARNING_RATE), loss='categorical_crossentropy', metrics=['accuracy'])
        metrics = model.evaluate(test_data, test_label, batch_size=16)
...
def predict():
    ...
    np.save("16_xception_label.npy", test_label)
    np.save("16_xception_predict.npy", preds)
